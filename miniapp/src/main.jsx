...
const L = {
  ru: {
    ...
    paymentNeeded: 'Бесплатный лимит исчерпан', consultPaymentText: 'Оплатите консультацию через Kaspi. После оплаты вернитесь в KORGAN, нажмите «Я оплатил» и выберите электронный чек Kaspi. KORGAN сам найдёт фискальный QR и проверит оплату через Kaspi ОФД.', payKaspi: 'Оплатить через Kaspi', uploadReceipt: 'Я оплатил', checkingReceipt: 'Проверяю оплату…', retryPaid: 'Повторить ответ без новой оплаты', paidSaved: 'Оплата подтверждена и сохранена. Повторно платить не нужно.',
    receiptUrlPlaceholder: 'Ссылка receipt.kaspi.kz — резервный способ', verifyPayment: 'Проверить ссылку (резерв)',
    documentPayment: 'Оплата документа', documentPaymentText: 'Оплатите документ через Kaspi. После оплаты вернитесь в KORGAN, нажмите «Я оплатил» и выберите электронный чек Kaspi. После проверки KORGAN сразу начнёт подготовку Word-документа.',
    waitingAdmin: 'Для старой платёжной заявки нужна повторная проверка того же фискального чека. Повторно платить не нужно.', paymentApproved: 'Оплата подтверждена', paymentApprovedText: 'Оплата сохранена. Если подготовка документа прервалась, её можно повторить без новой оплаты.', checkPayment: 'Обновить статус', startPaidGeneration: 'Повторить подготовку без оплаты',
    paymentRejected: 'Оплата не подтверждена. Проверьте, что загружен полный фискальный чек Kaspi.', manualCheck: 'Автоматическая проверка Kaspi', manualCheckSub: 'KORGAN извлекает фискальный QR из чека и проверяет через Kaspi ОФД сумму, получателя, время и уникальность. AI не принимает решение о факте оплаты.',
    ...
    helpText: 'Создайте дело, добавьте факты и материалы, задайте вопросы AI‑юристу. Оплата через Kaspi подтверждается по фискальному чеку через Kaspi ОФД. После подтверждения документ запускается автоматически, а статус оплаты сохраняется отдельно от генерации.',
  ...
  kk: {
    ...
    message: 'Заңдық сұрағыңызды жазыңыз…', checking: 'Құқық пен дереккөздер тексерілуде…', sources: 'Дереккөздер', freeRemaining: 'Қалған тегін кеңес', paymentNeeded: 'Тегін лимит аяқталды', consultPaymentText: 'Kaspi арқылы кеңес ақысын төлеңіз. Төлемнен кейін KORGAN-ға оралып, «Төледім» батырмасын басыңыз және Kaspi электрондық чегін таңдаңыз. KORGAN фискалдық QR-ды өзі тауып, Kaspi ОФД арқылы тексереді.', payKaspi: 'Kaspi арқылы төлеу', uploadReceipt: 'Төледім', checkingReceipt: 'Төлем тексерілуде…', retryPaid: 'Жаңа төлемсіз жауапты қайталау', paidSaved: 'Төлем расталды және сақталды. Қайта төлеу қажет емес.', receiptUrlPlaceholder: 'receipt.kaspi.kz сілтемесі — резервтік тәсіл', verifyPayment: 'Сілтемені тексеру (резерв)',
    documentPayment: 'Құжат төлемі', documentPaymentText: 'Kaspi арқылы құжат үшін төлеңіз. Төлемнен кейін KORGAN-ға оралып, «Төледім» батырмасын басыңыз және Kaspi электрондық чегін таңдаңыз. Тексеруден кейін KORGAN Word-құжатын бірден дайындай бастайды.', waitingAdmin: 'Ескі төлем өтінімі үшін сол фискалдық чекті қайта тексеру қажет. Қайта төлем жасамаңыз.', paymentApproved: 'Төлем расталды', paymentApprovedText: 'Төлем сақталды. Егер құжатты дайындау үзілсе, оны жаңа төлемсіз қайта бастауға болады.', checkPayment: 'Мәртебені жаңарту', startPaidGeneration: 'Жаңа төлемсіз қайта дайындау', paymentRejected: 'Төлем расталмады. Толық Kaspi фискалдық чегі жүктелгенін тексеріңіз.', manualCheck: 'Kaspi автоматты тексеруі', manualCheckSub: 'KORGAN чектен фискалдық QR-ды алып, Kaspi ОФД арқылы соманы, алушыны, уақытты және чектің бірегейлігін тексереді. AI төлем фактісін шешпейді.', filingReady: 'Беруге дайын', preliminary: 'Алдын ала құжат', verified: 'Тексерулер өтті', needsCheck: 'Тексеру қажет', quality: 'Сапа', status: 'Мәртебе', check: 'Тексеру', pricing: 'Тарифтер мен лимиттер', freePerDay: 'Күніне тегін кеңес', consultPrice: 'Лимиттен кейінгі кеңес', language: 'Тіл', deleteAll: 'Барлық деректерімді жою', dataControl: 'Деректер бақылауда', dataControlSub: 'Mini App бөлек API қолданады және production Telegram‑агентін өзгертпейді.', runtime: 'Заңдық ядро', secure: 'Қорғалған сақтау', refresh: 'Жаңарту', support: 'Техқолдау', helpText: 'Іс құрыңыз, фактілер мен материалдарды қосыңыз, AI‑заңгерге сұрақ қойыңыз. Kaspi төлемі фискалдық чек арқылы Kaspi ОФД-да тексеріледі. Расталған төлем құжат генерациясынан бөлек сақталады және қайта төлем талап етілмейді.',
...
function App() {
  ...
  const markConsultPaymentFailure = error => {
    if (error?.paymentConfirmed) {
      setConsultPayment(prev => prev ? ({ ...prev, paidPending: true }) : prev);
  ...
  const submitConsultReceiptUrl = async () => {
    ...
    catch (error) { markConsultPaymentFailure(error); }
  ...
  const retryPaidConsultation = async () => {
    ...
    catch (error) { markConsultPaymentFailure(error); } finally { setBusy(false); }
  };
