...
import { caseCardMeta, caseCardTitle } from './caseCard';
...
const DOCUMENTS = [
  ...
  // «Договор» — последний пункт раздела: порядок задаётся здесь, источником
  // данных списка, а не перестановкой в разметке или правилом CSS.
...
const L = {
  ru: {
    ...
    privacy: 'Конфиденциальность', privacySub: 'Согласие, язык и управление данными', connected: 'KORGAN подключён', connecting: 'Проверяю соединение…', down: 'Сервис временно недоступен', connectionLost: 'Связь прервалась. Работа продолжается — состояние обновится само.',
    ...
    automaticPayment: 'Оплата документа', automaticPaymentText: 'После успешной оплаты система автоматически подтвердит платёж и приступит к подготовке документа. Обычно это занимает несколько минут.', automaticPaymentSecurity: '',
...
function App() {
  ...
  if (view === 'cases') return <div className="app-shell"><Header go={go} title={t.myCases}/><main className="page">{banner}{notice && <div className="warning-note"><AlertTriangle size={17}/>{notice}</div>}{cases.length === 0 && <section className="analysis-card empty-card"><FolderOpen size={30}/><h2>{t.noCases}</h2><p>{t.noCasesSub}</p></section>}{cases.map(item => { const [title] = docText(item.document_type, language); return <button className="case-list-item" key={item.id} data-case-id={item.id} data-case-status={item.has_document ? 'completed' : ''} onClick={() => openCase(item)}><div className="case-badge"><Scale size={20}/></div><div><strong>{caseCardTitle(item, language)}</strong><small>{caseCardMeta(item, language)}</small></div><ChevronRight size={18}/></button>; })}<button className="primary wide" onClick={() => go('documents')}>{t.createNew}</button></main>{nav}</div>;
