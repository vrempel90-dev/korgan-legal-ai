...
function App() {
  ...
  // Одно нажатие открывает документ и оставляет его файл пользователю. Внутри
  // Telegram ссылку открывает сам клиент: `window.open` в его WebView либо
  // блокируется, либо уводит из Mini App без возврата.
  const deliverActiveDocument = async () => {
    if (!activeCase || busy) return; setBusy('deliver'); setNotice('');
    try {
      const tg = getTelegramWebApp();
      const result = await deliverDocument(activeCase.id, {
        insideTelegram: Boolean(tg),
        api: korganApi,
        openDocument: access => openDocumentForClient(access, { telegram: tg }),
      });
  ...
  if (view === 'documents') return <div className="app-shell"><Header go={go} title={t.selectDoc}/><main className="page">{banner}<div className="search"><Search size={18}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder={t.searchDoc}/></div>{pricing?.document_payments_enabled && <div className="price-note"><CreditCard size={16}/><span>{t.docPrice}: <strong>{money(pricing.document_price_kzt)}</strong> · {pricing?.document_payment_provider === 'tole' ? t.automaticPayment : t.manualCheck}</span></div>}<div className="section-kicker list-kicker">{t.documents}</div><div className="list-card">{filteredDocuments.map(item => { const [title, subtitle] = item[language]; const Icon = item.icon; return <button className="list-row" key={item.id} onClick={() => chooseDocument(item.id)}><span className="row-icon"><Icon size={20}/></span><span><strong>{title}</strong><small>{subtitle}</small></span><ChevronRight size={18}/></button>; })}</div></main>{nav}</div>;

  if (view === 'new-case') { const [title] = docText(selectedDocument, language); return <div className="app-shell"><Header go={go} title={t.newCase} back="documents"/><main className="page creation-page">{banner}<div className="big-title"><span className="eyebrow">{title}</span><h1>{t.tell}</h1><p>{t.tellSub}</p></div><textarea className="case-input" value={caseText} onChange={e => saveCaseText(e.target.value)} maxLength={8000} placeholder={t.placeholder}/><div className="input-meta"><Sparkles size={17}/><span>{caseText.length}/8000</span></div><label className="secondary wide"><Paperclip size={18}/>{t.addFile}<input className="hidden-input" multiple type="file" accept=".pdf,.docx,.txt,.jpg,.jpeg,.png,.webp" onChange={chooseInitialFiles}/></label>{pendingFiles.length > 0 && <div className="success-note">{t.selected}: {pendingFiles.map(f => f.name).join(', ')}</div>}{notice && <div className="warning-note"><AlertTriangle size={17}/>{notice}</div>}<button className="primary wide" disabled={(!caseText.trim() && !pendingFiles.length) || busy || !backendOk} onClick={createCase}>{busy ? <LoaderCircle className="spin" size={18}/> : <ArrowRight size={18}/>} {busy ? t.creating : t.create}</button></main>{nav}</div>; }
