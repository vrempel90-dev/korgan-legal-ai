...
ARTICLE_285 = (
    "Должник, обязанный совершить одно из двух или нескольких действий, имеет право выбора предмета исполнения обязательства."
)
ARTICLE_349 = (
    "Под нарушением обязательства понимается его неисполнение либо исполнение ненадлежащим образом, в том числе несвоевременное исполнение."
)
ARTICLE_27 = (
...
def test_correct_article_remains_filing_facing(grounding_corpus: Path) -> None:
    ...
    finalize_professional_claim(_case_context(), _research(correct), draft)
...
def test_both_companies_route_to_almaty_economic_court(grounding_corpus: Path) -> None:
    ...
    finalize_professional_claim(_case_context(), _research(), draft)
...
def test_company_vs_ordinary_person_is_not_forced_into_economic_court(grounding_corpus: Path) -> None:
    ...
    finalize_professional_claim(_case_context(mixed=True), _research(), draft)
...
def test_supplied_bank_and_filing_documents_clear_corresponding_actions(grounding_corpus: Path) -> None:
    ...
    finalize_professional_claim(_case_context(with_bank=True), _research(), draft)
