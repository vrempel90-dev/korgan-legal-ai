...
# --- Роутинг: кейс «долг по расписке» обязан попадать в профиль займа --------


def test_receipt_debt_routes_to_loan_profile() -> None:
    """Расписка — самостоятельное письменное доказательство передачи денег."""
    context = (
...
def test_wage_debt_is_not_captured_by_loan_profile() -> None:
    """«Долг по зарплате» обязан остаться трудовым спором."""
...
# --- Политика QA: что блокирует выдачу, а что становится пометкой ------------
...
def test_fabricated_pretrial_claim_is_blocking_not_a_gap() -> None:
    """«Выдуманная претензия» — это фабрикация, а не пробел в доказательствах."""
...
def test_soft_findings_become_needs_verification_inside_the_document() -> None:
    ...
    # Маркер в правовом обосновании переводит документ в PRELIMINARY DRAFT,
    # чтобы его нельзя было принять за готовый к подаче.
...
def test_legacy_string_raise_still_produces_a_reason() -> None:
    """Старый вызов с текстом не должен терять причину."""
...
def test_malformed_draft_payload_is_reported_as_technical() -> None:
    """Несовпадение схемы ответа модели — техническая, а не юридическая причина."""
