...
def test_thousand_separators_group_by_three(production_legal_calc) -> None:
    ...
    Раньше здесь ожидался [1]: разбор не мог совпасть с начала и возвращал
    хвостовую единицу. Это тот же дефект, из-за которого «12 34 567 тенге»
    давало 34 567, а «1234 567 тенге» — 567, и обрезок становился ценой иска
    и базой госпошлины. Теперь непонятная запись не разбирается вовсе:
    неверная сумма в документе дороже отказа её прочитать.
    """
    parse = production_legal_calc.parse_all_amounts_kzt

    assert parse("1 1 1 тенге") == []
