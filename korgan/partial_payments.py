...
# Нужен именно факт совершённого платежа, а не существительное «оплата».
# Иначе фраза-резюме «После частичной оплаты задолженность составила 6 750 000»
# ошибочно принимается за ещё один платёж без даты, и уже корректно разобранный
# платёж объявляется неоднозначным. Инфинитив «оплатить» намеренно не входит:
# обещание или обязанность оплатить не подтверждает перечисление денег.
_PAID_RE = re.compile(
    r"\b(?:оплатил\w*|оплачен\w*|уплатил\w*|уплачен\w*|погасил\w*|погашен\w*|"
    r"перечислил\w*|перечислен\w*|внес\w*|внёс\w*|вернул\w*|произведен\w*|"
    r"төле\w*|өтед\w*|аудар\w*)\b",
    re.IGNORECASE,
)
