...
def _allowed_article_numbers(claims: list[str]) -> set[str]:
    ...
    for claim in claims:
        ...
        for article in re.findall(r"(?i)(?:статья|статьи|ст\.)\s*(\d+(?:-\d+)?)", basis):
            allowed.add(article)
        if not allowed:
    ...
    async def _research_response_once(
        ...
        *,
        ...
        actual_urls = [
            ...
            if _is_adilet(url) and self._is_current_official_source(url)
        ]
    ...
    async def draft_response_to_claim(
        ...
        for raw in payload.get("legal_basis", []):
            ...
            if mentioned and not mentioned.issubset(allowed_articles):
                notes.append(
                    "Исключено из правового обоснования как неподтверждённая точная норма: " + item
                )
                continue
