...
from korgan.state_duty_final_hotfix import ProductionOpenAILegalService as _BaseProductionOpenAILegalService
from korgan.verified_openai import _actual_response_urls

LOGGER = logging.getLogger(__name__)

# Contracts are materially longer than claims. The previous production limits
# (5.2k + 6.2k retry) cut valid strict-JSON contracts in the middle of a string,
# which then surfaced to Telegram as a generic "cannot safely generate" error.
...
class ProductionOpenAILegalService(_BaseProductionOpenAILegalService):
    """Final KORGAN runtime with truncation-safe contract generation.

    Claim behavior remains untouched. Contract structured responses get enough
    output budget to finish the strict JSON in one pass, plus one larger retry
    only when the API explicitly returns an incomplete/truncated result or the
    returned text cannot be parsed as complete JSON.
    """
