...
async def health() -> dict[str, Any]:
    return {
        ...
        # Какой фронтенд этот бэкенд объявляет своим. Именно этот адрес
        # miniapp_telegram_launcher ставит кнопкой меню бота, то есть его и
        # открывает пользователь. Живых копий фронтенда несколько, и собраны они
        # из разных коммитов; без этого поля «исправление доехало до людей»
        # проверялось по той копии, которую проверяющий открыл сам, а не по той,
        # которую открывает клиент. Адрес не секрет: его видно в меню бота.
        "miniapp_url": os.getenv("MINIAPP_PUBLIC_URL", ""),
