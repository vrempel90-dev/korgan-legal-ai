...
_PENALTY_REQUEST_RE = re.compile(
    r"(?i)(?:взыск\w*|уплат\w*|оплат\w*|треб\w*|прошу|өндір\w*|төле\w*|талап\w*)"
)
...
def sanitize_draft_instructions(draft: Any) -> None:
    """Remove model-to-user instructions from every filing-facing party/requisite block.
...
def _penalty_amount(case_context: str, draft: ClaimDraft) -> int | None:
    """Find the actually claimed/calculated penalty, not the contractual cap.

    Candidate selection is deterministic and proximity-based. Sentences describing
    «не более 10% / лимит» are heavily penalized so the cap is not mistaken for
    the amount to request from the court.
    """
    ...
    for segment in segments:
        ...
        if not terms:
            continue
        for amount, start, end in _amount_occurrences(segment):
            distance = min(abs(start - term.start()) for term in terms)
            score = max(0, 8 - distance // 20)
            if _PENALTY_REQUEST_RE.search(segment):
                score += 5
            if re.search(r"(?i)(?:составил\w*|начисл\w*|итогов\w*|в\s+размере|в\s+том\s+числе)", segment):
                score += 4
...
def _penalty_should_be_in_prayer(case_context: str, draft: ClaimDraft) -> bool:
    ...
    if _PENALTY_RE.search(title) and _PENALTY_RE.search("\n".join(draft.facts)):
        return True
    for segment in re.split(r"(?<=[.!?])\s+|\n+", case_context or ""):
        if _PENALTY_RE.search(segment) and (
            "ТРЕБОВАНИЕ ИЗ ДОКУМЕНТА:" in segment or _PENALTY_REQUEST_RE.search(segment)
        ):
            return True
    return False


def complete_claim_relief_from_materials(case_context: str, draft: ClaimDraft) -> bool:
    """Restore a secondary contractual penalty only when the materials already claim it."""
    if not _penalty_should_be_in_prayer(case_context, draft):
        return False
    amount = _penalty_amount(case_context, draft)
    if amount is None:
        return False
    draft.requests.append(
        f"Взыскать с ответчика в пользу истца договорную неустойку в размере {format_kzt(amount)}."
    )
    LOGGER.info("UNIVERSAL_WORD_QUALITY restored_penalty amount=%s", amount)
    return True


def finalize_claim_for_release(case_context: str, draft: ClaimDraft) -> None:
    from korgan.professional_claim_finalizer import _recalculate_price

    sanitize_draft_instructions(draft)
    complete_claim_relief_from_materials(case_context, draft)
...
async def repair_pretrial_to_target(
    ...
    LOGGER.info("UNIVERSAL_WORD_QUALITY kind=pretrial target=10 issues_before=%d issues_after=%d", len(first), len(second))
    return repaired
...
def install_universal_word_quality_guard() -> None:
    ...
    async def guarded_claim(
        ...
        finalize_claim_for_release(case_context, draft)
        return draft
