...
class ProfessionalRKProductionService(UniversalQualityProductionService):
    ...
    async def draft_claim(
        ...
        prompt = (
            ...
            "7.1. Денежные суммы в требованиях не пиши цифрами вообще. Вместо суммы ставь плейсхолдер: {{principal_amount}} — основной долг, {{penalty_amount}} — неустойка, {{claim_price}} — цена иска, {{state_duty}} — госпошлина, {{total_claim}} — итог ко взысканию. Значения подставит детерминированный расчёт после генерации; собственное число модели в эти поля не попадает.\n"
