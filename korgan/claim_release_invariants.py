...
_UNOPPOSED_RE = re.compile(r"(?i)[,;.]?\s*(?:данн\w*\s+обстоятельств\w*\s+)?не\s+опровергнут\w*\s+ответчик\w*.*$")
...
def remove_circular_nonpayment_evidence(draft: ClaimDraft) -> None:
    ...
    if found:
        _add_filing_action(
            draft,
            "подтвердить отсутствие оплаты внешним/двусторонним доказательством: банковской выпиской, актом сверки либо иным первичным документом; собственная претензия истца не используется как доказательство неоплаты.",
        )
...
def flag_same_day_pretrial_risk(case_context: str, draft: ClaimDraft) -> None:
...
def enforce_claim_release_invariants(
    ...
    *,
    ...
    remove_circular_nonpayment_evidence(draft)
    restore_explicit_cost_request(case_context, draft, language=language)
    flag_same_day_pretrial_risk(case_context, draft)
