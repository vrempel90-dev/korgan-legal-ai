...
_RULES: tuple[RemedyRule, ...] = (
    ...
    RemedyRule(
        "contractual_debt",
        "погашение договорной задолженности",
        _rx(
            r"(?:взыска|уплат|погас|оплат|вернут)\w*.{0,120}(?:основн\w*\s+долг|долг|задолженн\w*|денежн\w*\s+сумм)",
            r"(?:основн\w*\s+долг|договорн\w*\s+задолженн\w*)",
            r"берешек\w*.{0,100}(?:төле|өндір|қайтар)",
        ),
        _rx(
            r"^(?!.*(?:за[её]м\w*|займ\w*|расписк\w*|\bв\s+долг\b|қарыз\w*)).*(?:договор\w*|шарт\w*|услуг\w*|қызмет\w*|работ\w*|жұмыс\w*|поставк\w*|жеткіз\w*|акт\w*|счет\w*|накладн\w*|обязательств\w*)",
        ),
        _rx(
            r"кредитор\w*.{0,120}треб\w*",
            r"должник\w*.{0,120}(?:обязан\w*|исполн\w*|уплат\w*|оплат\w*)",
            r"обязательств\w*.{0,120}(?:надлежащ\w*\s+образ\w*|исполн\w*|уплат\w*|оплат\w*)",
            r"надлежащ\w*.{0,120}исполн\w*\s+обязательств\w*",
            r"денежн\w*\s+обязательств\w*",
            r"(?:оплат|уплат)\w*.{0,100}(?:услуг\w*|работ\w*|товар\w*|поставк\w*)",
            r"міндеттем\w*.{0,120}(?:орында|төле)",
        ),
    ),
    RemedyRule(
        "prepayment_refund",
