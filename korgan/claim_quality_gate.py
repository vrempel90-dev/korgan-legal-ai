...
_ARTICLE_RE = re.compile(r"(?i)(?:статья|статьи|ст\.)\s*\d+")
...
def _has_known_value(case_context: str, value: str) -> bool:
    compact = re.sub(r"\s+", " ", value).strip().lower()
    return bool(compact and compact in re.sub(r"\s+", " ", case_context).lower())


def normalize_party_placeholders(draft: ClaimDraft) -> None:
    """Remove impossible requisites while keeping genuinely unknown fields visible."""
    defendant = _party_text(draft.defendant)
    if _ENTITY_RE.search(defendant):
        draft.defendant = [
            line for line in draft.defendant
            if not (
                "требует уточнения" in line.lower()
                and "фио ответчика" in line.lower()
            )
        ]
...
def _score_facts_and_amount(case_context: str, draft: ClaimDraft, issues: list[str]) -> float:
    ...
    if len([x for x in draft.facts if str(x).strip()]) < 3:
        score -= 0.5
        issues.append("фактическая часть слишком короткая для судебного иска")
    if _MONEY_RE.search(case_context) and not _MONEY_RE.search(draft.price_of_claim or ""):
        score -= 0.5
        issues.append("цена иска не отражает известную денежную сумму")
    if not draft.requests:
        score -= 0.5
        issues.append("отсутствует просительная часть")
    return max(0.0, score)
...
def _score_legal_basis(research: LegalResearch, draft: ClaimDraft, issues: list[str]) -> float:
    ...
    if research.verified_claims and not _ARTICLE_RE.search(basis):
        score -= 0.9
        issues.append("есть VERIFIED-нормы, но в правовом обосновании нет конкретных статей")
    ...
    if not research.verified_claims:
        score -= 0.7
        issues.append("нет подтвержденной материально-правовой основы")
    return max(0.0, score)
...
def _score_relief_coherence(case_context: str, draft: ClaimDraft, issues: list[str]) -> float:
    ...
    termination = any(x in text for x in ("отказ от договор", "расторг", "прекращен", "прекращён"))

    if prepayment and works and not_started and return_money and not termination:
        score -= 0.75
        issues.append("возврат предоплаты заявлен без ясной конструкции отказа/прекращения договора")

    if "неосновательн" in "\n".join(draft.legal_basis).lower() and not termination:
        score -= 0.35
        issues.append("неосновательное обогащение указано без объяснения отпадения договорного основания")
...
def _score_procedure(draft: ClaimDraft, issues: list[str]) -> float:
    ...
    if not court:
        score -= 0.75
        issues.append("не указан суд")
    elif "ТРЕБУЕТ УТОЧНЕНИЯ" in court.upper():
        score -= 0.45
        issues.append("точный суд не определен")

    duty = (draft.state_duty or "").upper()
    if not duty or "ТРЕБУЕТ" in duty:
        score -= 0.45
        issues.append("госпошлина не определена либо не обосновано освобождение")
...
def _score_evidence(case_context: str, draft: ClaimDraft, issues: list[str]) -> float:
    ...
    for source_marker, attachment_marker in known_pairs:
        if source_marker in case_context.lower() and attachment_marker not in attachments:
            score -= 0.2
            issues.append(f"в приложениях потеряно известное доказательство: {source_marker}")
    return max(0.0, score)
...
def _score_hygiene(draft: ClaimDraft, issues: list[str]) -> float:
    ...
    if any(token in body.lower() for token in forbidden):
        score -= 0.6
        issues.append("в судебном тексте остался служебный/чатовый текст")
    placeholders = _PLACEHOLDER_RE.findall(body)
    if len(placeholders) >= 4:
        score -= 0.4
        issues.append("в документе слишком много незаполненных служебных полей")
    elif placeholders:
...
def assess_claim_quality(case_context: str, research: LegalResearch, draft: ClaimDraft) -> ClaimQualityReport:
    ...
    return ClaimQualityReport(score=score, issues=list(dict.fromkeys(issues)), category_scores=categories)
