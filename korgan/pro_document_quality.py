...
# «low», а не «medium», сознательно. Составление без рассуждения («none»)
# давало конспект вместо иска — это чинил первый коммит. Но medium добавляет
# к каждому вызову десятки секунд, а документ должен выходить за одну-две
# минуты, а не за пять-шесть.
#
# Основной выигрыш во времени дало не снижение effort, а устранение ложных
# блокировок: раньше почти каждый иск шёл вторым кругом «черновик → правка →
# повторная валидация» и всё равно не выпускался. Один круг вместо двух
# экономит больше, чем любая настройка модели.
#
# Поднять до medium или high можно переменной KORGAN_DRAFT_REASONING_EFFORT
# без редеплоя кода.
DEFAULT_DRAFT_EFFORT = "low"
