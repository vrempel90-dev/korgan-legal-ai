...
def install_runtime_hotfix() -> None:
    ...
    from korgan.request_scope import request_scope_guard
    ...
    async def _send_claim(
        ...
        *,
        ...
        async def request_still_current() -> bool:
            current = await runtime.request_is_current(state, request_id, "claim")
            if not current:
                LOGGER.info("STALE_DOCUMENT_SUPPRESSED kind=claim request_id=%s", request_id)
            return current

        if not await request_still_current():
            return
