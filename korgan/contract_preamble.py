...
# «именуемое в дальнейшем «Заказчик»» и «именуемый далее Заказчик» — одинаково
# принятые формулировки. Проверка считала только первую, и договор с второй
# блокировался за оборот речи.
# «именуемое(-ый) в дальнейшем» из шаблона содержит скобку между словом и
# оборотом, поэтому в разделителе допускаются скобки и дефисы.
_ROLE_LABEL_RE = re.compile(
    r"именуем[\w()\u2010-\u2015-]*\s+(?:в\s+дальнейшем|далее)|бұдан\s+әрі",
    re.IGNORECASE,
)

# Признаки того, что стороной выступает организация или ИП. Только для них
# осмысленно требовать подписанта и основание его полномочий: гражданин,
# заключающий договор от своего имени, не действует «в лице» и не имеет
# «основания полномочий». Договор аренды квартиры, займа или продажи
# автомобиля между двумя физическими лицами блокировался именно на этом.
_ORGANISATION_RE = re.compile(
    r"\b(?:ТОО|АО|ЗАО|ОАО|РГП|РГУ|КГУ|КГП|ГКП|ПК|ЖК|КСК|ОО|ИП)\b|\bБИН\b|"
    r"товариществ\w*|акционерн\w*\s+обществ\w*|учрежден\w*|предприяти\w*|"
    r"индивидуальн\w*\s+предпринимател\w*|жауапкершілігі\s+шектеулі",
    re.IGNORECASE,
)
...
def preamble_defects(preamble: list[str]) -> list[str]:
    ...
    defects: list[str] = []
    if len(_ROLE_LABEL_RE.findall(text)) < 2:
        defects.append(
    ...
    # Подписант и основание его полномочий требуются только там, где стороной
    # выступает организация или ИП. Гражданин подписывает договор сам.
    if _ORGANISATION_RE.search(text):
