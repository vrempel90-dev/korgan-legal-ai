...
# Разряды разделяются пробелом по три цифры — так пишут сумму в договорах и
# исках. Явная группировка вместо «любая смесь цифр и пробелов» нужна не только
# для точности: прежний класс [\d\s]* допускал экспоненциальный откат, и строка
# вида «1 1 1 1 …» из материалов дела занимала процессор на минуты (рост
# квадратичный: 2000 токенов — 0.8 с, 4000 — 3.3 с при лимите ввода в 60 000
# символов). Здесь обе ветви проверяются за постоянное время и не пересекаются.
AMOUNT_PATTERN = re.compile(
    # Второй lookbehind запрещает начать разбор внутри числа, разделённого
    # пробелами. Без него «12 34 567 тенге» не совпадало с начала, разбор
    # перезапускался после пробела и возвращал 34 567, а «1234 567 тенге» —
    # 567: цена иска и госпошлина считались от обрезка вместо отказа.
    # Неверная сумма в иске дороже отказа разобрать её, поэтому fail-closed.
    r"(?<!\d)(?<!\d[\s ])("
    r"\d{1,3}(?:[\s ]\d{3})+(?:[.,]\d{1,2})?"   # 2 400 000,49
    r"|\d+(?:[.,]\d{1,2})?"                          # 2400000,49
