...
_NUMBER_WORD = (
    r"(?:один\w*|одн\w*|два|две|двух|три|трех|трёх|четыр\w*|пят\w*|шест\w*|сем\w*|"
...
_MONTH_NAME = (
    ...
    r"қаңтар\w*|ақпан\w*|наурыз\w*|сәуір\w*|мамыр\w*|маусым\w*|"
    r"шілде\w*|тамыз\w*|қыркүйек\w*|қазан\w*|қараша\w*|желтоқсан\w*)"
)

# Precise law belongs only to the source-bound verified block. Catch numeric and
# word-form rates/deadlines, calendar cutoffs and specific court-name phrases in
# all other client-visible fields.
_PRECISE_LAW_RE = re.compile(
    rf"(?i)(?:"
    rf"(?:стать[ьяеию]\w*|ст\.)\s*\d+"
    rf"|\b\d+(?:-\d+)?\s*[-\u2013]?\s*бап\b"
    rf"|\b\d+(?:[.,]\d+)?\s*%"
    rf"|\b\d+(?:[.,]\d+)?\s+(?:процент\w*|пайыз\w*)\b"
    ...
    rf"|\b(?:не\s+позднее|не\s+позже|до)\s+\d{{1,2}}\s+{_MONTH_NAME}(?:\s+\d{{4}}\s+года?)?"
    rf"|\b\d{{1,2}}[./-]\d{{1,2}}[./-]\d{{2,4}}\b[^\n]{{0,30}}(?:срок|подать|направить|жалоб|исков|заявлен|өтініш|шағым|талап)"
