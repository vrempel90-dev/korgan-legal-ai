...
def _add_lines(doc: Document, lines: list[str], *, bold: bool = False) -> None:
    for line in lines:
...
def build_response_to_claim_docx(draft: ResponseToClaimDraft) -> bytes:
    """Render a court-facing response without internal KORGAN QA chatter."""
    doc = Document()
    ...
    if draft.claimant:
        ...
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        p.add_run("Истец: [ТРЕБУЕТ УТОЧНЕНИЯ: ФИО/наименование и адрес]")
    ...
    if draft.defendant:
        ...
        p = doc.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.RIGHT
        p.add_run("Ответчик: [ТРЕБУЕТ УТОЧНЕНИЯ: ФИО/наименование и адрес]")
