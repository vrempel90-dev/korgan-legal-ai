...
Для нормы, приведённой **в кавычках как дословная цитата**, требование строже: совпадение с актуальным текстом должно быть симметричным, строка в строку. Частичное совпадение = `FAIL`. Цитата из непроверенной записи корпуса допускается только с видимой пометкой о необходимости сверки.

#### 8. Поцитатный проход перед выдачей

Проверка норм выполняется столько раз, сколько в документе ссылок на нормы, и **после** того, как черновик готов, а не только во время составления.

Наблюдаемый отказ: в одном документе статьи 953 и 960 ГК РК процитированы точно, а часть 2 статьи 166 ГПК РК — по отменённой редакции. Сбой происходит на уровне отдельной цитаты, поэтому документ, где «право в целом проверено», ничего не гарантирует.

`FAIL`, если хотя бы одна ссылка не прошла отдельную сверку. Относится к разделу C. В коде проход реализован как обязательный пост-процессинг (`korgan/citation_audit.py`), результат которого блокирует выдачу.

#### 9. Целостность текста

Готовый текст не должен содержать следов обрыва или склейки фрагментов: слово, приклеенное к закрывающей кавычке (`».евидно`), точка между словами без пробела, непарные кавычки, повторяющаяся пунктуация.

Такой след означает, что часть текста потеряна, — документ не выпускается. Относится к разделу H.
