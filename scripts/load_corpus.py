...
# headings such as «Статья 1» without a dot.  Horizontal whitespace is used
# deliberately so a body line can never be swallowed into the heading.
_ARTICLE_HEAD = re.compile(r"(?m)^[ \t]*Статья[ \t]+(\d+(?:-\d+)?)(?:\.)?[ \t]*([^\n]*)$")
